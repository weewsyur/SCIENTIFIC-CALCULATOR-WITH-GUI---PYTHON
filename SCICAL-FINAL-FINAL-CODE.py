...
def summation(expression, *args, **kwargs):
    ...
    for i in range(int(aeval.eval(str(start))), int(aeval.eval(str(end))) + 1):
        ...
        inner_aeval.symtable['summation'] = summation # Ensure recursion works
        inner_aeval.symtable['prod'] = prod
        inner_aeval.symtable[var] = i
        if len(args) > 3:
            total += summation(expression, *args[3:], **inner_aeval.symtable)
        else:
...
def factorial_func():
    global calc_operator
    try:
        # If the user just typed a number, compute its factorial directly
        if calc_operator.isdigit():
            result = math.factorial(int(calc_operator))
            calc_operator = str(result)
            text_input.set(result)
        else:
            # Try to safely evaluate any expression first
            value = aeval(calc_operator)
...
def prod(expression, *args, **kwargs):
    ...
    for i in range(start_val, end_val + 1):
        inner_aeval = Interpreter(symtable=aeval.symtable.copy())
        inner_aeval.symtable['summation'] = summation
        inner_aeval.symtable['prod'] = prod
        inner_aeval.symtable[var] = i
        if len(args) > 3:
            val = prod(expression, *args[3:], **inner_aeval.symtable)
        else:
            val = inner_aeval.eval(expression)
        # The direct call to prod/summation will not return None on success
        total *= val if val is not None else 1
    return total
...
def pre_parse_expression(expr):
    ...
    # And nested versions like ∑(expr,var1,start1,end1,var2,start2,end2)
    if expr.startswith('∑(') and expr.endswith(')'):
        inner = expr[2:-1]
        parts = [p.strip() for p in inner.split(',')]
        if len(parts) >= 4 and (len(parts) - 1) % 3 == 0:
            quoted_parts = [f"'{parts[0]}'"] + [f"'{p}'" if i % 3 == 1 else p for i, p in enumerate(parts[1:], 1)]
            return f"summation({','.join(quoted_parts)})"
    ...
    if expr.startswith('Π(') and expr.endswith(')'):
        ...
        if len(parts) >= 4 and (len(parts) - 1) % 3 == 0:
            quoted_parts = [f"'{parts[0]}'"] + [f"'{p}'" if i % 3 == 1 else p for i, p in enumerate(parts[1:], 1)]
            return f"prod({','.join(quoted_parts)})"
...
factorial_btn = Button(tk_calc, button_params, text='n!',
                       command=factorial_func).grid(
